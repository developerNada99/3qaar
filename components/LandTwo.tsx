"use client";

import React from 'react';
import Image from 'next/image';
import { motion } from 'framer-motion';
import { Cairo } from 'next/font/google';
import { Property } from '@/app/actions/properties';
import MediaGallery from '@/components/Mediagallery';

const cairo = Cairo({
  subsets: ['arabic'],
  weight: ['400', '600', '700', '800'],
});

type Props = {
  offers: Property[];
};

const LandTwo = ({ offers }: Props) => {
  return (
    <section className="relative w-full min-h-screen overflow-hidden" dir="rtl">

      <Image
        src="/landtwo.jpg"
        alt="أراضي خام في الطائف"
        fill
        priority
        quality={100}
        className="object-cover object-center"
        sizes="100vw"
      />

      <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/60 to-black/20" />

      <div className={`${cairo.className} relative z-10 flex flex-col justify-end min-h-screen pb-12 px-6 sm:px-12 md:px-20`}>

        {/* عنوان السكشن */}
        <div className="mb-10">
          <span className="text-white/40 text-xs tracking-[0.4em] font-light mb-3 block">05</span>

          <h2 className="text-white text-4xl sm:text-5xl md:text-6xl font-black leading-tight mb-4">
            أراضي{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-l from-[#7a9ed4] to-[#274b97]">
              خام
            </span>
          </h2>

          <div className="w-16 h-0.5 bg-gradient-to-l from-[#7a9ed4] to-[#274b97] rounded-full mb-5" />

          <p className="text-white/75 text-base sm:text-lg leading-relaxed max-w-xl mb-6">
            أراضي خام بأسعار منافسة في مواقع استراتيجية مميزة بالطائف، فرص استثمارية
            نادرة لمن يبحث عن التوسع والبناء في أكثر المناطق نمواً وتطوراً.
          </p>

          <span className="px-4 py-2 rounded-full border border-white/30 text-white/80 text-sm font-semibold backdrop-blur-sm bg-white/5">
            أراضي خام
          </span>
        </div>

        {/* العروض من Sanity */}
        {offers && offers.length > 0 && (
          <div>
            <div className="flex items-center gap-3 mb-5">
              <div className="h-px flex-1 bg-white/10" />
              <span className="text-white/50 text-xs tracking-[0.3em] font-medium uppercase">العروض المتاحة</span>
              <div className="h-px flex-1 bg-white/10" />
            </div>

            <div className="flex flex-wrap gap-5 pb-4">
              {offers.map((offer) => (
                <motion.div
                  key={offer._id}
                  initial={{ opacity: 0, x: -20 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.5 }}
                  className="w-full sm:w-[340px] bg-white/8 backdrop-blur-md border border-white/15 rounded-3xl overflow-hidden p-3"
                >
                  {/* الميديا */}
                  {offer.media && offer.media.length > 0 && (
                    <MediaGallery media={offer.media} />
                  )}

                  {/* التفاصيل */}
                  <div className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="text-white/40 text-[10px] tracking-widest uppercase font-medium">عرض حصري</p>
                        <h3 className="text-white text-base font-black">
                          {offer.number ? `قطعة رقم ${offer.number}` : offer.title}
                        </h3>
                        {(offer.plan || offer.location) && (
                          <p className="text-[#7a9ed4] text-xs font-semibold">
                            {offer.plan}{offer.plan && offer.location ? " — " : ""}{offer.location}
                          </p>
                        )}
                      </div>
                      {offer.status && (
                        <span className="px-2.5 py-1 rounded-full bg-[#274b97] text-white text-[10px] font-bold shrink-0">
                          {offer.status}
                        </span>
                      )}
                    </div>

                    {offer.details && offer.details.length > 0 && (
                      <div className="grid grid-cols-3 gap-2 mb-3">
                        {offer.details.map((d, j) => (
                          <div key={j} className="bg-white/8 border border-white/10 rounded-xl p-2 text-center">
                            <p className="text-white/40 text-[9px] font-medium mb-0.5">{d.label}</p>
                            <p className="text-white text-[11px] font-bold">{d.value}</p>
                          </div>
                        ))}
                      </div>
                    )}

                    {offer.description && (
                      <p className="text-white/50 text-xs leading-relaxed mb-3">{offer.description}</p>
                    )}

                    {offer.twitterLink && (
                      <a
                        href={offer.twitterLink}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-[#7a9ed4] text-xs mb-3 hover:text-white transition-colors duration-200"
                      >
                        <svg viewBox="0 0 24 24" fill="none" className="w-3 h-3" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
                        </svg>
                        عرض طبيعة الأرض على X
                      </a>
                    )}

                    <div className="flex items-center justify-between gap-3 pt-3 border-t border-white/10">
                      <div>
                        <p className="text-white/40 text-[10px]">المطلوب</p>
                        <p className="text-white text-lg font-black">
                          {offer.price && offer.price !== "للتفاوض" ? `${offer.price} ﷼` : offer.price || "—"}
                        </p>
                        <p className="text-white/30 text-[9px]">{offer.priceNote || "صافي"}</p>
                      </div>
                      <a
                        href={offer.whatsappLink || "https://wa.me/966559002060"}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-1.5 px-4 py-2.5 rounded-2xl bg-white text-[#274b97]
                                   text-xs font-black hover:bg-[#f0f4fb] transition-colors duration-200 shrink-0"
                      >
                        <svg className="w-4 h-4 text-green-600" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.967-.94 1.165-.173.199-.347.223-.644.075-.297-.149-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.447-.52.149-.174.198-.298.298-.497.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.5-.67-.51-.173-.01-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
                        </svg>
                        استفسر
                      </a>
                    </div>
                  </div>
                </motion.div>
              ))}
            </div>
          </div>
        )}

      </div>
    </section>
  );
};

export default LandTwo;